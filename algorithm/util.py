...
class Util(object):
    ...
    def gain_flags_cluster(self, index):
        if index == 0:
            #分支型编译选项
            flags_branch = ['-branch-prob','-hotcoldsplit', '-jump-threading','-tailcallelim']
            #页错误编译选项
            ...
            #计算密集型程序编译选项 
            flags_compute = ['-instcombine', '-reassociate', '-gvn', '-sccp', '-licm', '-loop-unroll', '-loop-vectorize', '-slp-vectorizer', '-irce', '-indvars', '-aggressive-instcombine']
            # 针对内存访问和 dTLB_store_misses
            flags_io = ['-sroa','-mem2red','-loop-distribute','-loop-fusion','-loop-interchange']
            ...
            flags_icache = ['-globalopt', '-dce', '-globaldce', '-instcombine', '-mergefunc', '-mergereturn', '-block-placement', '-hotcoldsplit', '-partial-inliner', '-inline']
